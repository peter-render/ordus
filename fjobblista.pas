...
uses
  ...
  Vcl.Wwdbdatetimepicker, Vcl.wwdblook, Vcl.Mask,System.UITypes;
...
begin
  inherited;
  if MessageDlg('Vill du klarmarkera "' + fdquery1.fieldbyname('Arbete')
...
begin
  ...
  ELSE if AFieldName = 'Arbete' then
    sstr := 'Order by Arbete'
  else if AFieldName = 'Namn' then
    sstr := 'Order by Namn'
  ...
  if sstr <> '' then
    ...
    fdquery1.sql.Add(' ' + sstr + ' ' + iif(ascending, 'asc', 'desc') +
      iif((AFieldName <> 'KlartSenast'), ', Klartsenast ', ' '));
