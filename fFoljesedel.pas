...
uses
  ...
  vcl.gtQRXport, vcl.gtxClasses, REST.Backend.EMSServices, REST.Backend.EMSFireDAC;
...
type
  TfrmFoljesedel = class(TForm)
    ...
    procedure wwDBGrid1QuerySortField(Sender: TObject; var SortFieldName: string; var IsDescending: Boolean);
  private
...
begin
  ...
  xFilename := FoldernameFix(ftgsystemvalue('pdf.folder.foljesedel', '')) + 'Följesedel_' + qryFoljesedelId.asstring;
  ...
  begin
    ...
    Mail.Subject := 'Följesedel (' + qryFoljesedelId.asstring + ')';
    Mail.Body := 'Hej!' + chr(13) + chr(10) + 'Följesedel som PDF-bilaga';
    // + chr(13) + chr(10) +  'Vi tackar för uppdraget.';
...
begin
  ...
  begin
    ...
    if (qryFoljesedelLevereradDatum.asstring <> '') and
      (messagedlg('Leveransdatumet är redan ifyllt - vill du ändra det till dagens datum?', mtConfirmation,
...
procedure TfrmFoljesedel.wwDBGrid1QuerySortField(Sender: TObject; var SortFieldName: string; var IsDescending: Boolean);
begin
if sortfieldname <> '' then
showmessage(sortfieldname);
end;
...
begin
  ...
  begin
    ...
    parambyname('@FöljesedelId').Value := qryFoljesedelId.asInteger;
    execproc;
  end;
...
begin
  ...
  begin
    if qryFoljesedelAntalrader.asInteger = 0 then
      ...
      showmessage('Bara följesedlar utan rader kan tas bort!');
...
begin
  ...
  begin
    ...
    if (qryFoljesedelArkiverad.asBoolean = false) and
      (messagedlg('Vill du arkivera följesedeln?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
    begin
      edit;
